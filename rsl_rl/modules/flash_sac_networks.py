...
class FlashSACActor(nn.Module):
    ...
    def get_mean_and_std(self, observations: torch.Tensor, training: bool) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        x = self.post_norm(x)
        return self.predictor.get_mean_and_std(x)
...
class FlashSACDoubleCritic(nn.Module):
    ...
    def forward(self, observations: torch.Tensor, actions: torch.Tensor, training: bool) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
...
class FlashSACTemperature(nn.Module):
    def __init__(self, initial_value: float = 0.01):
        super().__init__()
        self.log_temp = nn.Parameter(torch.tensor([math.log(max(initial_value, 1e-8))], dtype=torch.float32))
